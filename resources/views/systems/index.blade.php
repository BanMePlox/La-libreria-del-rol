        ...
        @foreach ($systems as $sistema)
            <div class="container col-sm-4 text-warning text-center border border-3">
                <a href="{{ 'systems/' . $sistema->id }}"><img src="{{ asset('storage') . '/' . $sistema->file_path }}"
                        alt="Portada sistema" width="300px" height="300px" class=""></a>
                <br>
                ...
                @if (Auth::check())
                    @if (Auth::user()->admin)
                        ...
                        <a href="{{ 'delete/' . $sistema['id'] }}"
                            onclick="return confirm('Quieres eliminar el sistema?')"><i class="fa-solid fa-eraser"></i></a>
                    @endif
                @endif
            </div>
        @endforeach
